...
#HE: Add your own info here as well, it's not just me anymore :)
...
#HE: This needs to also be translated, or, if all will have RStudio prompt them to install those packages, entirely remove it
```{r}
# Just provide the barrage of install.packages and load the libraries only when needed
# install.packages("pacman")
pacman::p_load(tidyverse, tidytext, stopwords, topicmodels, word2vec, broom)
```
...
tokenize_speech <- function(.data, col = speech){
  ...
  .data |>
    ...
    # TODO: Remove also words containing digits
    #Remove stopwords and words containing digits
    filter(!tokens %in% stops, !grepl("\\d", tokens))
}
```
...
calculate_tf_idf <- function(.data, term = tokens, document = id, n = n){
  .data |>
    bind_tf_idf({{term}}, {{document}}, {{n}})
}

tf <- calculate_tf_idf(tokens_df)
```
...
```{r}
#HE: This looks like a lot of code - the question is if we want to show it to people or if it confuses them more? Especially since DTMs are the one thing R is insanely nice to us with
...
fit_lda_model <- 
  function(dtm){
    ...
    #Beta might not be available in R
    #HE: Might? Or is it not at all? I can't check because somehow R complains when I try to compile the topicmodels package, so I can't check the docs
    #beta = 0.01
...
#HE: Again Python stuff, please adapt
...
#HE: Here is again gensim library, instead of the R one
...
#HE: Add your own info here as well! :)
